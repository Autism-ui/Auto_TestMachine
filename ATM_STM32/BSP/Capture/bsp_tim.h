 ...
 *
 *  instructions:
 *	In terms of configuration, four PWM channels (TIM1) need to be turned on, with two channels
 capturing one PWM input. If it is also a HAL library, it needs to turn on the input capture
 interrupt (note: do not delete the callback function); In the case of the standard library, the two
 Captor object callbacks can be used by putting them into the library's interrupt service function.

	In addition,LEDDevice obj is only use when author testing the function.please don't be confuse
 of it.You can just ignore it when you use this file.
 *
 *	API to use:
 *	1.Captor_init()
...
typedef struct TIM_Captor {
	TIM_HandleTypeDef
		*htimx; /*! < TIM Time Base Handle Structure definition,This property is used for
					  compatibility with the HAL library. If you are using the standard library or
					  another library, this item can be assigned a value of 0	>*/

	uint32_t Cap_val1; /* !< The value of CCRx captured by Tim .
							 This parameter can be acquired by function 'HAL_TIM_ReadCapturedValue'
						  if you use HAL library  > */
	...
	uint32_t (*TIM_Capture)(struct TIM_Captor *,
							uint8_t Level); /* !< Obtain the CCR value of PWM input > */
	void (*Duty_cal)(struct TIM_Captor *p_Dev,
					 TIM_ActiveChannel); /* !< Duty cycle calculation of PWM waveform > */
	void (*Frequency_cal)(struct TIM_Captor *p_Dev,
						  TIM_ActiveChannel);	 /* !< Frequency calculation of PWM waveform > */
	void (*Pulse_cal)(struct TIM_Captor *p_Dev); /* !< Pulse width calculation of PWM waveform > */

} TIM_Captor_t, *p_TIM_Captor;

// Use this code to externally initialize and view the structure object
typedef TIM_Captor_t Captor_viewer;
