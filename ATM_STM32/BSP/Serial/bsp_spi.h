...
typedef struct spi_Device_t{									//spi设备对象

    //属性
    SPI_HandleTypeDef *spix;
		spi_sel_t_e spi_id;                                         //设备id,在同一个SPI设备上（SPI1或者SPI2上可以有很多设备，这里用ID来区分他们）
    uint8_t cs_status;									//片选线的状态
...
typedef spi_err_type_e spi_err;     //别称外部用这个类型进行观测，而不要直接使用spi_err_type_e

/*--- Public variable declarations ----------------------------------------------------*/
//extern SPI_Device SPI[SPI_NUM];
extern spi_err spi_err_state;        //全局，外部可以观测spi设备状态,要观测直接在该.c文件中定义spi_err spi_err_state变量
